...
namespace TeoGames.Mesh_Combiner.Scripts.Editor.MenuItems {
	public class Assets : AssetModificationProcessor {
		...
		[MenuItem("Assets/Dynamic Mesh Combiner/Mark all meshes as Read-Write", false, 400)]
		public static void MarkAsReadable() => Utils.GetPrefabs(Selection.objects).ForEach(o => Utils.SetReadableStatus(o, true));

		[MenuItem("Assets/Dynamic Mesh Combiner/Unmark all meshes as Read-Write", false, 401)]
		public static void UnmarkAsReadable() => Utils.GetPrefabs(Selection.objects).ForEach(o => Utils.SetReadableStatus(o, false));
